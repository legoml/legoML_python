...
def UNIT_TEST___WALTER_2015___RoboAI___ProbSet1_Q1():
    ...
    p_T1.pprint()
    p_T1___answer = dfmf(dict.fromkeys(('T1',)),
                         dict(mappings={fdict(T1=0): -log(0.5),
                                        fdict(T1=1): -log(0.5)}))
    p_T1___check = dfmf_allclose(p_T1, p_T1___answer)
    print('Prob(T1) check:', p_T1___check)

    p_T2 = (p_C.multiply(p_T2_on_C)).marginalize(('C',))
    p_T2.pprint()
    p_T2___answer = dfmf(dict.fromkeys(('T2',)),
                         dict(mappings={fdict(T2=0): -log(0.54),
                                        fdict(T2=1): -log(0.46)}))
    p_T2___check = dfmf_allclose(p_T2, p_T2___answer)
    print('Prob(T2) check:', p_T2___check)

    p_T1_T2 = (p_C.multiply(p_T1_on_C, p_T2_on_C)).marginalize(('C',))
    p_T1_T2.pprint()
    p_T1_T2___answer = dfmf(dict.fromkeys(('T1', 'T2')),
                            dict(mappings={fdict(T1=0, T2=0): -log(0.312),
                                           fdict(T1=0, T2=1): -log(0.188),
                                           fdict(T1=1, T2=0): -log(0.228),
                                           fdict(T1=1, T2=1): -log(0.272)}))
    p_T1_T2___check = dfmf_allclose(p_T1_T2, p_T1_T2___answer)
    print('Prob(T1, T2) check:', p_T1_T2___check)
    ...
    p_T1_T2_on_C_equal_n___answer = dfmf(dict.fromkeys(('C', 'T1', 'T2')),
                                         dict(mappings={fdict(T1=0, T2=0): -log(0.3),
                                                        fdict(T1=0, T2=1): -log(0.2),
                                                        fdict(T1=1, T2=0): -log(0.3),
                                                        fdict(T1=1, T2=1): -log(0.2)}),
                                         conditions=dict(C='n'))
    p_T1_T2_on_C_equal_n___check = dfmf_allclose(p_T1_T2_on_C_equal_n, p_T1_T2_on_C_equal_n___answer)
    print('Prob(T1, T2 | C = n) check:', p_T1_T2_on_C_equal_n___check)
    ...
    p_T1_T2_on_C_equal_n_and_D_equal_1___answer = dfmf(dict.fromkeys(('C', 'T1', 'T2', 'D')),
                                                       dict(mappings={fdict(T1=0, T2=0): -log(0.0),
                                                                      fdict(T1=0, T2=1): -log(0.0),
                                                                      fdict(T1=1, T2=0): -log(0.6),
                                                                      fdict(T1=1, T2=1): -log(0.4)}),
                                                       conditions=dict(C='n', D=1))
    p_T1_T2_on_C_equal_n_and_D_equal_1___answer.pprint()
    p_T1_T2_on_C_equal_n_and_D_equal_1___check = dfmf_allclose(p_T1_T2_on_C_equal_n_and_D_equal_1,
                                                               p_T1_T2_on_C_equal_n_and_D_equal_1___alternative,
