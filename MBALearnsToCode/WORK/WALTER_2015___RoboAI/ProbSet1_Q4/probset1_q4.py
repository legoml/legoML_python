...
def UNIT_TEST___WALTER_2015___RoboAI___ProbSet1_Q4():
    ...
    forward_2___check = dfmf_allclose(forward[2], alpha_2, atol=1e-3)

    alpha_3 = dfmf(dict.fromkeys((('X', 3), ('Z', 0), ('Z', 1), ('Z', 2), ('Z', 3))),
                   dict(mappings={fdict({('X', 3): 0, ('Z', 0): 0, ('Z', 1): 0, ('Z', 2): 1, ('Z', 3): 0}):
                                      -log(0.0365),
                                  fdict({('X', 3): 1, ('Z', 0): 0, ('Z', 1): 0, ('Z', 2): 1, ('Z', 3): 0}):
                                      -log(0.0156)}),
                   scope={('Z', 0): 0, ('Z', 1): 0, ('Z', 2): 1, ('Z', 3): 0})
    forward_3___check = dfmf_allclose(forward[3], alpha_3, atol=1e-2)
    ...
    beta_3 = dfmf(dict.fromkeys((('X', 3),)),
                  dict(mappings={fdict({('X', 3): 0}): -log(1.0),
                                 fdict({('X', 3): 1}): -log(1.0)}),
                  conditions={('X', 3): None})
    backward_3___check = dfmf_allclose(backward[3], beta_3)

    beta_2 = dfmf(dict.fromkeys((('X', 2), ('Z', 3))),
                  dict(mappings={fdict({('X', 2): 0, ('Z', 3): 0}): -log(0.56),
                                 fdict({('X', 2): 1, ('Z', 3): 0}): -log(0.38)}),
                  conditions={('X', 2): None},
                  scope={('Z', 3): 0})
    backward_2___check = dfmf_allclose(backward[2], beta_2)

    beta_1 = dfmf(dict.fromkeys((('X', 1), ('Z', 2), ('Z', 3))),
                  dict(mappings={fdict({('X', 1): 0, ('Z', 2): 1, ('Z', 3): 0}): -log(0.1888),
                                 fdict({('X', 1): 1, ('Z', 2): 1, ('Z', 3): 0}): -log(0.2464)}),
                  conditions={('X', 1): None},
                  scope={('Z', 2): 1, ('Z', 3): 0})
    backward_1___check = dfmf_allclose(backward[1], beta_1)

    beta_0 = dfmf(dict.fromkeys((('X', 0), ('Z', 1), ('Z', 2), ('Z', 3))),
                  dict(mappings={fdict({('X', 0): 0, ('Z', 1): 0, ('Z', 2): 1, ('Z', 3): 0}): -log(0.1103),
                                 fdict({('X', 0): 1, ('Z', 1): 0, ('Z', 2): 1, ('Z', 3): 0}): -log(0.0798)}),
                  conditions={('X', 0): None},
                  scope={('Z', 1): 0, ('Z', 2): 1, ('Z', 3): 0})
    backward_0___check = dfmf_allclose(backward[0], beta_0, atol=1e-3)
    ...
    infer_0 = dfmf(dict.fromkeys((('X', 0), ('Z', 0), ('Z', 1), ('Z', 2), ('Z', 3))),
                   dict(mappings={fdict({('X', 0): 0}): -log(0.4 * 0.1103),
                                  fdict({('X', 0): 1}): -log(0.1 * 0.0798)}),
                   conditions={('Z', 0): 0, ('Z', 1): 0, ('Z', 2): 1, ('Z', 3): 0}).normalize()
    infer_0.pprint()
    infer_0___check = dfmf_allclose(infer_state[0], infer_0, atol=1e-3)

    infer_1 = dfmf(dict.fromkeys((('X', 1), ('Z', 0), ('Z', 1), ('Z', 2), ('Z', 3))),
                   dict(mappings={fdict({('X', 1): 0}): -log(0.216 * 0.1888),
                                  fdict({('X', 1): 1}): -log(0.046 * 0.2464)}),
                   conditions={('Z', 0): 0, ('Z', 1): 0, ('Z', 2): 1, ('Z', 3): 0}).normalize()
    infer_1.pprint()
    infer_1___check = dfmf_allclose(infer_state[1], infer_1)

    infer_2 = dfmf(dict.fromkeys((('X', 2), ('Z', 0), ('Z', 1), ('Z', 2), ('Z', 3))),
                   dict(mappings={fdict({('X', 2): 0}): -log(0.0287 * 0.56),
                                  fdict({('X', 2): 1}): -log(0.0949 * 0.38)}),
                   conditions={('Z', 0): 0, ('Z', 1): 0, ('Z', 2): 1, ('Z', 3): 0}).normalize()
    infer_2.pprint()
    infer_2___check = dfmf_allclose(infer_state[2], infer_2, atol=1e-3)

    infer_3 = dfmf(dict.fromkeys((('X', 3), ('Z', 0), ('Z', 1), ('Z', 2), ('Z', 3))),
                   dict(mappings={fdict({('X', 3): 0}): -log(0.0365),
                                  fdict({('X', 3): 1}): -log(0.0156)}),
                   conditions={('Z', 0): 0, ('Z', 1): 0, ('Z', 2): 1, ('Z', 3): 0}).normalize()
    infer_3.pprint()
    infer_3___check = dfmf_allclose(infer_state[3], infer_3, atol=1e-2)
    ...
    for t in range(T + 1):
        ...
        map_joint_distributions += [hmm.map_joint_distributions(all_z[t])]
        map_joint_distributions[t].pprint()

    map_0 = dfmf(dict.fromkeys((('X', 0), ('Z', 0))),
                 dict(mappings={fdict({('X', 0): 0, ('Z', 0): 0}): -log(0.4)}),
                 scope={('Z', 0): 0})
    map_0___check = dfmf_allclose(map_joint_distributions[0], map_0)

    map_1 = dfmf(dict.fromkeys((('X', 0), ('Z', 0), ('X', 1), ('Z', 1))),
                 dict(mappings={fdict({('X', 0): 0, ('Z', 0): 0, ('X', 1): 0, ('Z', 1): 0}): -log(0.192)}),
                 scope={('Z', 0): 0, ('Z', 1): 0})
    map_1___check = dfmf_allclose(map_joint_distributions[1], map_1)

    map_2 = dfmf(dict.fromkeys((('X', 0), ('Z', 0), ('X', 1), ('Z', 1), ('X', 2), ('Z', 2))),
                 dict(mappings={fdict({('X', 0): 0, ('Z', 0): 0, ('X', 1): 0, ('Z', 1): 0,
                                       ('X', 2): 1, ('Z', 2): 1}): -log(0.0614)}),
                 scope={('Z', 0): 0, ('Z', 1): 0, ('Z', 2): 1})
    map_2___check = dfmf_allclose(map_joint_distributions[2], map_2, atol=1e-3)

    map_3 = dfmf(dict.fromkeys((('X', 0), ('Z', 0), ('X', 1), ('Z', 1), ('X', 2), ('Z', 2), ('X', 3), ('Z', 3))),
                 dict(mappings={fdict({('X', 0): 0, ('Z', 0): 0, ('X', 1): 0, ('Z', 1): 0, ('X', 2): 1, ('Z', 2): 1,
                                       ('X', 3): 0, ('Z', 3): 0}): -log(0.0147)}),
                 scope={('Z', 0): 0, ('Z', 1): 0, ('Z', 2): 1, ('Z', 3): 0})
    map_3___check = dfmf_allclose(map_joint_distributions[3], map_3, atol=1e-2)

    map_state_sequence = hmm.map_state_sequences(all_z[3])
