...
class RunEKF:
    ...
    def plot(self):
        ...
        axarr[0].update_ball_and_player_plots(T,Error[:,0],'r-')
        axarr[0].update_ball_and_player_plots(T,sigma*sqrt(self.VAR[:,0]),'b--')
        axarr[0].update_ball_and_player_plots(T,-sigma*sqrt(self.VAR[:,0]),'b--')
        axarr[0].set_title('X error')
        axarr[0].set_ylabel('Error (m)')
        
        axarr[1].update_ball_and_player_plots(T,Error[:,1],'r-')
        axarr[1].update_ball_and_player_plots(T,sigma*sqrt(self.VAR[:,1]),'b--')
        axarr[1].update_ball_and_player_plots(T,-sigma*sqrt(self.VAR[:,1]),'b--')
        axarr[1].set_title('Y error')
        axarr[1].set_ylabel('Error (m)')
        
        axarr[2].update_ball_and_player_plots(T,degrees(unwrap(Error[:,2])),'r-')
        axarr[2].update_ball_and_player_plots(T,sigma*degrees(unwrap(sqrt(self.VAR[:,2]))),'b--')
        axarr[2].update_ball_and_player_plots(T,-sigma*degrees(unwrap(sqrt(self.VAR[:,2]))),'b--')
