...
def UNIT_TEST___WALTER_2015___RoboAI___MidTerm_Q1():
    
    # Number of Time Periods
    T = 2

    state_prior = dfmf(dict.fromkeys((('S', 0),)),
                       dict(mappings={fdict({('S', 0): 1}): -log(1 / 3),
                                      fdict({('S', 0): 2}): -log(1 / 3),
                                      fdict({('S', 0): 3}): -log(1 / 3)}))
    state_transition_template = dfmf(dict.fromkeys((('S', -1), ('S', 0))),
                                     ...
                                     conditions={('S', -1): None})
    observation_template = dfmf(dict.fromkeys((('S', 0), ('Z', 0))),
    ...
    alpha_0 = dfmf(dict.fromkeys((('S', 0), ('Z', 0))),
    ...
    alpha_1 = dfmf(dict.fromkeys((('S', 1), ('Z', 0), ('Z', 1))),
    ...
    alpha_2 = dfmf(dict.fromkeys((('S', 2), ('Z', 0), ('Z', 1), ('Z', 2))),
    ...
    forward_2___check = dfmf_allclose(forward[2], alpha_2, atol=1e-2)
    ...
    beta_1 = dfmf(dict.fromkeys((('S', 1), ('Z', 2))),
    ...
    infer_0 = dfmf(dict.fromkeys((('S', 0), ('Z', 0), ('Z', 1), ('Z', 2))),
                   dict(mappings={fdict({('S', 0): 1}): -log(.0038),
                                  fdict({('S', 0): 2}): -log(.0143),
                                  fdict({('S', 0): 3}): -log(.0106)}),
                   conditions={('Z', 0): 2, ('Z', 1): 3, ('Z', 2): 1}).normalize()
    infer_0.pprint()
    infer_0___check = dfmf_allclose(infer_state[0], infer_0, atol=1e-1)

    infer_1 = dfmf(dict.fromkeys((('S', 1), ('Z', 0), ('Z', 1), ('Z', 2))),
                   dict(mappings={fdict({('S', 1): 1}): -log(.007),
                                  fdict({('S', 1): 2}): -log(.0081),
                                  fdict({('S', 1): 3}): -log(.0136)}),
                   conditions={('Z', 0): 2, ('Z', 1): 3, ('Z', 2): 1}).normalize()
    infer_1.pprint()
    infer_1___check = dfmf_allclose(infer_state[1], infer_1, atol=1e-2)

    infer_2 = dfmf(dict.fromkeys((('S', 2), ('Z', 0), ('Z', 1), ('Z', 2))),
                   dict(mappings={fdict({('S', 2): 1}): -log(.0176),
                                  fdict({('S', 2): 2}): -log(.0043),
                                  fdict({('S', 2): 3}): -log(.0068)}),
                   conditions={('Z', 0): 2, ('Z', 1): 3, ('Z', 2): 1}).normalize()
    infer_2.pprint()
    infer_2___check = dfmf_allclose(infer_state[2], infer_2, atol=1e-1)
    ...
    map_0 = dfmf(dict.fromkeys((('S', 0), ('Z', 0))),
                 dict(mappings={fdict({('S', 0): 2, ('Z', 0): 2}): -log(.7 / 3)}),
                 scope={('Z', 0): 2})
    map_0___check = dfmf_allclose(map_joint_distributions[0], map_0)

    map_1 = dfmf(dict.fromkeys((('S', 0), ('Z', 0), ('S', 1), ('Z', 1))),
                 dict(mappings={fdict({('S', 0): 2, ('Z', 0): 2, ('S', 1): 2, ('Z', 1): 3}): -log(.0327)}),
                 scope={('Z', 0): 2, ('Z', 1): 3})
    map_1___check = dfmf_allclose(map_joint_distributions[1], map_1, atol=1e-3)

    map_2 = dfmf(dict.fromkeys((('S', 0), ('Z', 0), ('S', 1), ('Z', 1), ('S', 2), ('Z', 2))),
                 dict(mappings={fdict({('S', 0): 3, ('Z', 0): 2, ('S', 1): 3, ('Z', 1): 3,
                                       ('S', 2): 1, ('Z', 2): 1}): -log(.0048)}),
                 scope={('Z', 0): 2, ('Z', 1): 3, ('Z', 2): 1})
    map_2___check = dfmf_allclose(map_joint_distributions[2], map_2)
    ...
    return forward_0___check & forward_1___check & forward_2___check &\
